...
from app.domain.resume import MasterResume
...
def profile_source_text(profile: ProfileContext) -> str:
    ...
    because it does not appear in the free-text resume body — and, for the same
    reason, so is the structured master resume. A technology named only in a
    structured experience is still something the candidate provided, and a guard
    that flagged it would be crying invention over the user's own data.

    The four original parts are composed exactly as they always were, and the
    structured block is appended only when it has content: a profile with no
    structured entries hashes to the byte-identical string it did before, so no
    stored fingerprint moves and no stored draft turns stale on upgrade.
    """
    parts = [
        ...
        MasterResume.from_profile(profile).structured_text,
    ]
