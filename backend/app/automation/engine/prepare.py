...
class PrepareMixin(EngineBase):
    ...
    async def _prepare_application(
        ...
        # Imported here, not at module scope: `app.services` reaches the API's
        # error types, which reach `app.automation` — the engine can only see
        # the service layer once both packages are initialised.
        ...
        async with session_scope() as session:
            ...
            # The engine is the other door that creates applications, so it is
            # the other place that has to give one its own resume version. An
            # application already holding a version keeps it: the user may have
            # edited it, and re-preparing the form is no reason to overwrite
            # their wording.
            owner = await session.get(User, user_id)
            if owner is not None:
                await resume_service.ensure_version(session, owner, application)
