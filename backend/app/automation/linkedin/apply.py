...
class EasyApplyModal:
    ...
    async def read_questions(self) -> list[FormQuestion]:
        ...
        groups = await self._groups(modal)
        self._fields = {}
        questions: list[FormQuestion] = []

        for index, group in enumerate(groups):
            field = await self._describe_group(group, index)
            if field is None:
                continue
    ...
    async def _groups(self, modal: Locator) -> list[Locator]:
        for selector in sel.EasyApply.FORM_GROUPS:
            try:
                ...
                continue
        # Markup variant without group wrappers: treat each control as its own group.
        found: list[Locator] = []
        for selector in ("input", "select", "textarea"):
            try:
                found.extend(await modal.locator(selector).all())
            except PlaywrightError:
                continue
        return found

    async def _describe_group(self, group: Locator, index: int) -> _Field | None:
        radios = group.locator(sel.EasyApply.RADIO_INPUT[0])
        try:
        ...
        if radio_count:
            return await self._describe_radio_group(group, radios, index)

        control, kind = await self._first_control(group)
        if control is None or kind is None:
            return None

        label = await self._label_for(group, control)
        field_id = await self._field_id(control, index)
        options: list[str] = []
        if kind == "select":
            options = await self._select_options(control)
        ...
        return _Field(question=question, control=control, group=group)

    async def _describe_radio_group(
        self, group: Locator, radios: Locator, index: int
    ) -> _Field | None:
        try:
            first = radios.first
        ...
        try:
            for radio in await radios.all():
                text = await self._radio_option_label(group, radio)
                if text:
                    options.append(text)
        ...
        legend = await self._first_text(group, ("css=legend", "css=label"))
        question = FormQuestion(
            field_id=name or f"group:{index}",
            label=clean_text(legend) or f"Field {index + 1}",
            kind="radio",
            options=options,
            required=await self._is_required(group, radios.first, legend),
            current_value=checked,
        )
        return _Field(question=question, control=radios, group=group)

    async def _first_control(self, group: Locator) -> tuple[Locator | None, QuestionKind | None]:
        """Pick the single meaningful control of a group, ignoring file inputs."""
        probes: tuple[tuple[str, QuestionKind], ...] = (
            ("select", "select"),
            ("textarea", "textarea"),
        ...
        for selector, kind in probes:
            try:
                locator = group.locator(selector)
                if await locator.count():
                    return locator.first, kind
            except PlaywrightError:
                continue
        # A bare `input` with no type attribute behaves as text.
        try:
            bare = group.locator('input:not([type="file"]):not([type="hidden"])')
            if await bare.count():
                return bare.first, "text"
        except PlaywrightError:
            pass
        return None, None

    async def _label_for(self, group: Locator, control: Locator) -> str:
    ...
    async def _is_required(self, group: Locator, control: Locator, label: str) -> bool:
    ...
    async def _field_id(self, control: Locator, index: int) -> str:
    ...
    async def fill_and_advance(
        ...
        for step in range(1, _MAX_STEPS + 1):
            ...
            if cover_letter_pending:
                if await self._fill_cover_letter(cover_letter_pending, questions):
                    draft.cover_letter_attached = True
                    cover_letter_pending = None

            step_unanswered = await self._fill_step(questions, pending, by_label, draft)
    ...
    async def _fill_cover_letter(self, content: str, questions: list[FormQuestion]) -> bool:
        """Paste the cover letter into the free-text box, when this step has one."""
        candidates = [
            question
            for question in questions
            if question.kind == "textarea"
            and any(marker in _normalize(question.label) for marker in sel.EasyApply.COVER_LETTER_LABELS)
        ]
        if not candidates:
