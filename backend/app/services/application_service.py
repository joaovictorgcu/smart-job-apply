...
def _application_query() -> Select[tuple[Application]]:
    return select(Application).options(
        ...
        # The resume version hangs off the job (one job, one application, one
        # version), and every read of an application reports which resume it
        # uses — so it is loaded here rather than lazily, one query per read.
        selectinload(Application.job).selectinload(Job.tailored_resume),
        selectinload(Application.events),
    )


def _resume_focus(application: Application) -> list[str]:
    """What this application's resume version leads with, if it has one.

    Reads through the eagerly loaded relationship; a version derived before the
    structured resume existed has an empty `focus`, which reads correctly as
    "nothing to say about the emphasis".
    """
    job = application.job
    row = job.tailored_resume if job is not None else None
    if row is None or not row.document:
        return []
    return list(row.focus or [])
...
async def get_or_create_for_job(session: AsyncSession, user: User, job_id: int) -> Application:
    ...
    await session.flush()
    # Every application gets its own version of the resume at birth, derived
    # from the master as it stands right now. `ensure_version` returns None
    # rather than raising when the master is still empty: a new account has
    # nothing to derive from, and refusing to create the application over that
    # would be absurd.
    await resume_service.ensure_version(session, user, application)
    logger.info(
...
async def list_applications(
    ...
    *,
    ...
    result = await session.execute(
        select(Application)
        # The list reports each application's resume emphasis, which lives on
        # the job's version row: two selectin queries for the whole page.
        .options(selectinload(Application.job).selectinload(Job.tailored_resume))
        .where(*conditions)
...
async def mark_applied(
    ...
    if payload.confirm is not True:
        raise PreconditionFailedError(
            "Recording a manual application requires explicit confirmation: send 'confirm': true."
        )
...
def to_application_read(application: Application) -> ApplicationRead:
    ...
    read.resume_focus = _resume_focus(application)
    return read
